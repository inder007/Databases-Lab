...
public:
	...
	void twoWaySort(DiskFile &inputFile, MainMemory &memory, int flag, vector<DiskFile> &temp);
...
void ExtMergeSort :: twoWaySort(DiskFile &inputFile, MainMemory &memory, int flag, vector<DiskFile> &temp){
    ...
    int t=(memory.totalFrames-flag)/flag;
...
int main()
{
	int k; //if flag=0 normal else double buffering
	cin>>DISK_PAGE_SIZE;
    ...
    int totalpages=ceil(n/DISK_PAGE_SIZE);
    ...
    vector<DiskFile> temp;

	//call 2 way externalmerge sort
	e.twoWaySort(f,mm, k, temp);
    ...
    while(1){
        ...
        if(r<=0){
            cout<<"Invalid Pass No. Give pass no greater than 0"<<endl;
            continue;
        }
        if(y<=0){
            cout<<"Invalid page no. Give page no greater than 0"<<endl;
            continue;
        }
        if(y>temp[r-1].totalPages){
            cout<<"Invalid Page Number"<<endl;
            continue;
        }
        // cout<<temp[r].data[x];
        for(int i=0;i<temp[r-1].data[y-1].validEntries;i++){
            cout<<temp[r-1].data[y-1].arr[i];
            if(i<temp[r-1].data[y-1].validEntries-1){
                cout<<", ";
            }
            else{
                cout<<endl;
