...
public:
	...
	int twoPower(int p);
	void printHashTable();
    int checkNeedForDoubling(int val);
...
int ExtendibleHashing::checkNeedForDoubling(int val){
    for(int i=0;i<this->dir.size();i++){
        if(i!=val && this->dir[i]->bucket.size() <= this->bucketSize){
            return 0;
        }
    }
    return 1;
...
void ExtendibleHashing::insertElement(int val){
	int bucketIndex = this->getBucketIndex(val);
    if(this->dir[bucketIndex]->bucket.size() > this->bucketSize || this->dir[bucketIndex]->bucket.size() < this->bucketSize){
        this->dir[bucketIndex]->bucket.push_back(val);
    }
    else{
        
        int flag = this->checkNeedForDoubling(bucketIndex);
        if(flag){
...
void ExtendibleHashing::splitBucket(int bucketIndex){
	int localDepthPower = twoPower(this->globalDepth-1);
