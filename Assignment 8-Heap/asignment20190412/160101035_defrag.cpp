#define DISK_FILE_SIZE 10000000
...
class Page
{
  ...
  public:
    ...
    Page()
    {
        ...
        // this->spaceLeft = DISK_PAGE_SIZE - (sizeof(this->dirSlotCount) + this->arr.size() * DIR_ENTRY_LENGTH);
...
class DiskFile
{
  public:
    ...
    void defragment(Node **head_ref);
    void defragmentPage(Node **head_ref, int n);
};

void sortDirectory(Node* &temp){
    for(int i=0;i<temp->data.arr.size();i++){
        ...
        temp->data.spaceLeft+=gap;
        if(temp->data.arr[i].valid==false){
            temp->data.spaceLeft+=temp->data.arr[i].length+DIR_ENTRY_LENGTH;
            temp->data.dirSlotCount--;
            temp->data.arr.erase(temp->data.arr.begin()+i);
            i--;
        }
        // else{
        //     temp->data.arr[i].length=temp->data.arr[i].var_length;
        // }
        
    }
    for(int i=temp->data.arr.size()-1;i>=0;i--){
        for(int j=0;j<i;j++){
            if(temp->data.arr[j].length>temp->data.arr[j+1].length){
                swap(temp->data.arr[j],temp->data.arr[j+1]);
            }
        }
    }
    int start=1;
    for(int i=0;i<temp->data.arr.size();i++){
        temp->data.arr[i].start=start;
        start+=temp->data.arr[i].length;
    }
}

void DiskFile::defragmentPage(Node **head_ref, int n){
    int count=1;
    Node* temp=*head_ref;
    while(temp!=NULL){
        if(count==n){
    ...
    sortDirectory(temp);
    // temp->data.spaceLeft = 
    // sort(temp->data.arr.begin(),temp->data.arr.end());
}

void DiskFile::defragment(Node **head_ref){
    Node* temp=*head_ref;
    while(temp!=NULL){
        sortDirectory(temp);  
        temp=temp->next;
...
{
    if (rec_length > (DISK_PAGE_SIZE - (1 * sizeof(int) + DIR_ENTRY_LENGTH)))
    {
        printf("Record length should be less than effective allowable Page size %d, where the maximum Page size is %d\n", (int)(DISK_PAGE_SIZE - (1 * sizeof(int) + DIR_ENTRY_LENGTH)), DISK_PAGE_SIZE);
    ...
    while (last != NULL)
    {
        ...
        // if ((last->data.dirSlotCount == 1) && (last->data.arr[0].valid == false) && (last->data.arr[0].length==0))
        if (last->data.dirSlotCount == 0)
        {
            last->data.arr.resize(1,DirectoryEntry());
            last->data.arr[0].id = rec_id;
            last->data.arr[0].length = rec_length;
            last->data.arr[0].start = 1;
                ...
                if (last->data.arr[i].valid == false)
                {
                    ...
                    if (space_available)
                    {
                        if(i==last->data.arr.size()-1){
                            if(rec_length < last->data.arr[i].length){
...
void DiskFile ::printPages(struct Node *node)
{
    ...
    printf("DiskFile traversal, displaying Pages with Records \n <ID,Start,Length,Valid> :\n");
    int count = 0;
    while (node != NULL)
    {
        count = count + 1;
        ...
        for (int i = 0; i < node->data.arr.size(); i++)
        {
            ...
            // int gap=node->data.arr[i].length-node->data.arr[i].var_length;
            if (i != node->data.arr.size() - 1)
                printf("<%d, %d, %d, %s> ", node->data.arr[i].id, node->data.arr[i].start, node->data.arr[i].length, node->data.arr[i].valid ? "true" : "false");
            else
                printf("<%d, %d, %d, %s>", node->data.arr[i].id, node->data.arr[i].start, node->data.arr[i].length, node->data.arr[i].valid ? "true" : "false");
        }
        printf("}\n");
...
int main()
{
    ...
    while (true)
    {
        ...
        case 5:
            if(create_diskFile){
                int n;
                cout<<"Give page to defragment"<<endl;
                cin>>n;
                d.defragmentPage(&d.nodePointer,n);
            }
            else{
                printf("You can not view Pages & Records without creating the initial DiskFile.\n");
            }
